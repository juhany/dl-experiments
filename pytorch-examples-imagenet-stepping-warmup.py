...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        acc1 = validate(val_loader, model, criterion, ngpus_per_node, args)
...
def train(train_loader, model, criterion, optimizer, epoch, ngpus_per_node, args):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if (i % args.print_freq == 0) and (args.rank % ngpus_per_node == 0):
            print('Epoch: [{0}][{1}/{2}]\t'
...
def validate(val_loader, model, criterion, ngpus_per_node, args):
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
...
def adjust_learning_rate(optimizer, epoch, args):
    ...
    if ((epoch < 5) and (args.batch_size * args.world_size >= 512)):
